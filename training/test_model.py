...
CLASSIFIER_PATH = '/home/tensorflow/wca-dev/fast-MPN-COV/Results/Finetune-printer_data-mpncovresnet50-MPNCOV-reproduce-lr1.2e-3-bs10/model_best.pth.tar'
IMG_PATH = '/home/tensorflow/wca-dev/fast-MPN-COV/data/printer_data/test'

CLASSES = ['step0', 'step1', 'step10', 'step11', 'step2', 'step3', 'step4', 'step5', 'step6', 'step7', 'step8', 'step9']
...
def main():
    ...
    freezed_layer = 0
    model = Newmodel(representation, num_classes, freezed_layer)
    ...
    for label in os.listdir(IMG_PATH):
        print(label)
        for img_name in os.listdir(os.path.join(IMG_PATH, label)):
            img = Image.open(os.path.join(IMG_PATH, label, img_name))
            transformed = transform(img).cuda()

            output = model(transformed[None, ...])
            _, pred = output.topk(1, 1, True, True)
            pred = pred.t()

            if CLASSES[pred[0][0]] == label:
                good += 1
            else:
                bad += 1
